...
interface MRFCollectionSelectedBoxProps {
  batches: WasteBatch[];
  selectedBatch: WasteBatch | null;
  setSelectedBatch: (batch: WasteBatch) => void;
}

export const MRFCollectionSelectedBox: React.FC<MRFCollectionSelectedBoxProps> = ({
  batches = [],
  selectedBatch,
  setSelectedBatch,
}) => {
  return (
    <div className="w-full flex justify-center mt-8 pb-6">
      <div className="grid lg:grid-cols-3 md:grid-cols-2 grid-cols-1 gap-5 w-full p-4 h-[600px] overflow-y-auto">
        ...
        {[...Array(Math.max(0, 6 - batches.length))].map((_, index) => (
          <div
            key={`empty-${index}`}
            className="bg-gray-100 h-[307px] rounded-lg shadow-lg flex items-center justify-center"
          >
            <p className="text-gray-500">No batch available</p>
          </div>
        ))}
      </div>
    </div>
