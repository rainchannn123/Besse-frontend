...
interface ListedReadyChartProps {
  data: {
...
const ListedReadyChart: React.FC<ListedReadyChartProps> = ({ data }) => {
      ...
      {hasData ? (
        <>
          ...
          <div className="text-center mt-3 text-xs text-[#7a5f41]">
            Active: {data.total.toFixed(2)} tons
          </div>
        </>
      ) : (
        <div className="flex items-center justify-center h-[180px]">
          <p className="text-[#8b7355] text-sm text-center">No active auctions</p>
        </div>
      )}
    </div>
  );
...
export default ListedReadyChart;