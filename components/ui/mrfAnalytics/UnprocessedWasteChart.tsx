...
interface UnprocessedWasteChartProps {
  data: {
...
const DonutChart: React.FC<{ data: { name: string; value: number; color: string; percentage: number }[]; total: number }> = ({ data, total }) => {
  ...
  return (
    <div className="flex flex-col items-center">
      <div 
        className="relative w-[140px] h-[140px] rounded-full overflow-hidden shadow-md" 
...
const UnprocessedWasteChart: React.FC<UnprocessedWasteChartProps> = ({ data }) => {
      ...
      {hasData ? (
        <>
          ...
          <div className="text-center mt-3 text-xs text-[#7a5f41]">
            Total: {data.total.toFixed(2)} tons pending
          </div>
        </>
      ) : (
        <div className="flex items-center justify-center h-[180px]">
          <p className="text-[#8b7355] text-sm text-center">No unprocessed waste</p>
        </div>
      )}
    </div>
  );
...
export default UnprocessedWasteChart;