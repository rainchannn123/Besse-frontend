...
import {
  AdminMonitorOverviewData,
  AdminPlayerGameRecord,
  AdminPlayerRow,
  AdminPlayerStatus,
} from "@/types/admin";
...
interface ResumeLiveMonitorState {
  roomCode: string;
  roomName: string;
  isReview: boolean;
  reviewUntilMs?: number;
}

const formatReviewTimeLeft = (remainingMs: number): string => {
...
export default function AdminMonitorPage() {
  ...
  const [overview, setOverview] = useState<AdminMonitorOverviewData | null>(
    null,
  );
  const [search, setSearch] = useState("");
  const [statusFilter, setStatusFilter] = useState<"all" | AdminPlayerStatus>(
    "all",
  );
  const [actionLoadingUserId, setActionLoadingUserId] = useState<string | null>(
    null,
  );
  const [expandedHistoryUserId, setExpandedHistoryUserId] = useState<
    string | null
  >(null);
  const [historyRecords, setHistoryRecords] = useState<
    Record<string, AdminPlayerGameRecord[]>
  >({});
  ...
  const [resumeLiveMonitor, setResumeLiveMonitor] =
    useState<ResumeLiveMonitorState | null>(null);
  const [reviewNowMs, setReviewNowMs] = useState(Date.now());

  const roomStatusHistoryRef = useRef<Record<string, Room["status"]>>({});
  ...
  const getAdminToken = (): string | null => {
    return (
      localStorage.getItem("admin_monitor_token") ||
  ...
  const handleCreateRoom = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `${API_URL}/matchmaking/rooms/admin-create`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
  ...
  const handleStartGameForTeam = async (
    sessionId: string,
    teamName: string,
  ) => {
    ...
    try {
      ...
      if (roomsData.success && roomsData.data?.rooms) {
        for (const room of roomsData.data.rooms) {
          const isInRoom = room.teams?.some(
  ...
  const resolveResumeLiveMonitor = useCallback((roomList: Room[]) => {
    const startedRoom = roomList.find((room) => room.status === "started");

    if (startedRoom) {
      setAdminLiveMonitorActive(startedRoom.roomCode, startedRoom.roomName);
      setResumeLiveMonitor({
        roomCode: startedRoom.roomCode,
        roomName: startedRoom.roomName,
        isReview: false,
      });
      return;
    }

    const storedMonitor = readAdminLiveMonitorState();
    if (!storedMonitor) {
      setResumeLiveMonitor(null);
      return;
    }

    const matchingRoom = roomList.find(
      (room) => room.roomCode.toUpperCase() === storedMonitor.roomCode,
    );

    const reviewUntilMs = storedMonitor.reviewUntilMs;

    if (typeof reviewUntilMs !== "number") {
      setResumeLiveMonitor({
        roomCode: storedMonitor.roomCode,
        roomName:
          matchingRoom?.roomName ||
          storedMonitor.roomName ||
          storedMonitor.roomCode,
        isReview: false,
      });
      return;
    }

    if (!isReviewWindowActive(storedMonitor)) {
      clearAdminLiveMonitorState();
      setResumeLiveMonitor(null);
      return;
    }

    if (!matchingRoom || matchingRoom.status !== "completed") {
      clearAdminLiveMonitorState();
      setResumeLiveMonitor(null);
      return;
    }
  ...
  useEffect(() => {
    const nextStatusMap: Record<string, Room["status"]> = {};

    for (const room of rooms) {
      nextStatusMap[room.roomCode] = room.status;

      const previousStatus = roomStatusHistoryRef.current[room.roomCode];

      if (room.status === "started") {
        setAdminLiveMonitorActive(room.roomCode, room.roomName);
      }

      if (previousStatus === "started" && room.status === "completed") {
        setAdminLiveMonitorReviewWindow(room.roomCode, room.roomName);
      }
    }

    roomStatusHistoryRef.current = nextStatusMap;
    resolveResumeLiveMonitor(rooms);
  }, [rooms, resolveResumeLiveMonitor]);

  useEffect(() => {
    if (!resumeLiveMonitor?.isReview || !resumeLiveMonitor.reviewUntilMs) {
      return;
    }

    const reviewUntilMs = resumeLiveMonitor.reviewUntilMs;

    const tick = () => {
      const now = Date.now();
      setReviewNowMs(now);

      if (now >= reviewUntilMs) {
        clearAdminLiveMonitorState();
        setResumeLiveMonitor(null);
      }
    };
  ...
  const reviewTimeLeftLabel = useMemo(() => {
    if (!resumeLiveMonitor?.isReview || !resumeLiveMonitor.reviewUntilMs) {
      return null;
    }

    const remainingMs = resumeLiveMonitor.reviewUntilMs - reviewNowMs;
    if (remainingMs <= 0) {
      return null;
    }

    return formatReviewTimeLeft(remainingMs);
  }, [resumeLiveMonitor, reviewNowMs]);
          ...
          players: prev.players.map((p) =>
            p.userId === player.userId
              ? {
                  ...p,
                  currentSession: null,
                  status: "offline" as AdminPlayerStatus,
                }
              : p,
          ),
        };
  ...
  const handleToggleHistory = async (player: AdminPlayerRow) => {
    ...
    try {
      ...
      const response = await adminService.getPlayerHistory(
        player.userId,
        20,
        0,
      );
      ...
      addNotification({
        message:
          error.response?.data?.message || "Failed to load player history",
        type: "error",
      });
      setExpandedHistoryUserId(null);
    } finally {
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-[#f5efe2] flex items-center justify-center">
        <p className="text-[#5b3a1f] text-xl font-semibold">
  ...
  if (!overview) {
    return (
      <div className="min-h-screen bg-[#f5efe2] flex flex-col items-center justify-center gap-4 p-6">
        <p className="text-[#5b3a1f] text-lg font-semibold">
        ...
        <div className="mx-auto max-w-[1600px] space-y-6">
          ...
          <header className="flex flex-wrap items-center justify-between gap-4 rounded-2xl border border-[#d3c4ad] bg-[#fff9ef] p-5 shadow-sm">
            <div>
              ...
              <p className="text-sm md:text-base text-[#6d4b2a]">
                Track live sessions, team composition, role assignments, and
                force-reset players when needed.
              </p>
            </div>
              ...
              {resumeLiveMonitor && (
                <div className="flex flex-col items-start">
                  <button
                    onClick={() =>
                      router.push(
                        `/dashboard/admin-game-room/${resumeLiveMonitor.roomCode}/live`,
                      )
                    }
                    className="rounded-lg bg-[#2c5b8e] px-4 py-2 text-white font-semibold hover:bg-[#224a73]"
                  >
                    {resumeLiveMonitor.isReview
                      ? `Open Live Review · ${resumeLiveMonitor.roomCode}`
                      : `Resume Live Monitor · ${resumeLiveMonitor.roomCode}`}
                  </button>
                  {reviewTimeLeftLabel && (
                    <span className="mt-1 text-xs text-[#6d4b2a]">
                      Review time left: {reviewTimeLeftLabel}
                    </span>
                  )}
                </div>
              )}
          ...
          <section className="grid grid-cols-2 md:grid-cols-4 xl:grid-cols-8 gap-3">
            ...
            <div className={cardStyle}>
              ...
              <p className="text-2xl font-bold text-[#8a6f13]">
                {overview.summary.waitingRoom}
              </p>
            </div>
            <div className={cardStyle}>
              <p className="text-xs uppercase tracking-wide text-[#7a5f41]">
              ...
              <p className="text-2xl font-bold text-[#2c5b8e]">
                {overview.summary.roleSelection}
              </p>
            </div>
            <div className={cardStyle}>
              <p className="text-xs uppercase tracking-wide text-[#7a5f41]">
              ...
              <p className="text-2xl font-bold text-[#6b3da3]">
                {overview.summary.pairing}
              </p>
            </div>
            <div className={cardStyle}>
              <p className="text-xs uppercase tracking-wide text-[#7a5f41]">
              ...
              <p className="text-2xl font-bold text-[#505863]">
            ...
            <div className={cardStyle}>
              ...
              <p className="text-2xl font-bold text-[#4f2d14]">
                {overview.summary.activeLobbies}
              </p>
            </div>
            <div className={cardStyle}>
              <p className="text-xs uppercase tracking-wide text-[#7a5f41]">
          ...
          <section className="rounded-2xl border border-[#d3c4ad] bg-[#fff9ef] p-5 shadow-sm space-y-4">
            <div className="flex flex-wrap items-center gap-3">
              ...
              <select
                value={statusFilter}
                onChange={(e) =>
                  setStatusFilter(e.target.value as "all" | AdminPlayerStatus)
                }
                  ...
                  {filteredPlayers.map((player) => (
                    <Fragment key={player.userId}>
                      <tr className="rounded-lg bg-white shadow-[0_1px_6px_rgba(52,37,12,0.08)]">
                          ...
                          <span
                            className={`rounded-full px-2 py-1 text-xs font-semibold ${
                              player.accountType === "admin"
                                ...
                                : player.accountType === "educator"
                                  ? "bg-purple-100 text-purple-800"
                                  : player.accountType === "spectator"
                        ...
                        <td className="px-3 py-3 align-top">
                          <span
                            className={`rounded-full px-2 py-1 text-xs font-semibold ${statusColors[player.status]}`}
                          >
                            {statusLabel[player.status]}
                          </span>
                          <p className="text-xs mt-2 text-[#6d4b2a]">
                            Socket:{" "}
                            {player.hasActiveSocketConnections
                              ? "🟢 Connected"
                              : "🔴 No live socket"}
                          </p>
                        </td>
                        <td className="px-3 py-3 align-top text-sm text-[#4f2d14]">
                          {player.roleInSession ? (
                            <span className="capitalize font-semibold text-[#33552C]">
                              {player.roleInSession}
                            </span>
                          ) : (
                        ...
                        <td className="px-3 py-3 align-top text-sm text-[#4f2d14]">
                          ...
                          <p className="text-xs text-[#6d4b2a] mt-2">
                            Competitors:
                          </p>
                      ...
                      {expandedHistoryUserId === player.userId && (
                        <tr className="bg-[#f8f3ea]">
                          <td colSpan={9} className="px-4 py-3">
                            ...
                            ) : !historyRecords[player.userId] ||
                              historyRecords[player.userId].length === 0 ? (
                              <p className="text-sm text-[#6d4b2a]">
                                No past game records found.
                              </p>
                            ) : (
                              <div className="overflow-x-auto">
                                <table className="w-full text-sm border-separate border-spacing-y-1">
                                  ...
                                  <tbody>
                                    {historyRecords[player.userId].map(
                                      (record) => (
                                        <tr
                                          ...
                                          <td className="px-2 py-1">
                                            <span
                                              className={`rounded-full px-2 py-0.5 text-xs font-semibold ${
                                                record.gameStatus === "won"
                                                  ? "bg-emerald-100 text-emerald-900"
                                                  : record.gameStatus === "lost"
          ...
          <section className="rounded-2xl border border-[#d3c4ad] bg-[#fff9ef] p-5 shadow-sm">
            <h2 className="text-xl font-bold text-[#4f2d14] mb-4">
              Created Game Rooms
            </h2>
                          ...
                          <td className="px-4 py-3">
                            <div className="flex gap-2">
                              <button
                                onClick={() =>
                                  router.push(
                                    `/dashboard/admin-game-room/${room.roomCode}`,
                                  )
                                }
                              ...
                              {canStart && (
                                <button
                                  onClick={() =>
                                    handleStartGameFromAdmin(room.roomCode)
                                  }
                              ...
                              {room.status === "started" && (
                                <button
                                  onClick={() =>
                                    router.push(
                                      `/dashboard/admin-game-room/${room.roomCode}/live`,
                                    )
                                  }
